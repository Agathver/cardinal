...
function normalizeMarkers(markers) {
  ...
  // map to hold markers
  var markersMap = Object.create(null);

  for (var i in markers) {
    var marker = markers[i];
    if(!markersMap[marker.line]) {
      markersMap[marker.line] = [];
    }

    if(marker.column) {
      markersMap[marker.line].push(marker.column);
    }
  }
  return markersMap;
}
  ...
  for (var i = 0; i < linesLen; i++) {
    ...
    if(columns.length) {
      ...
      for(var m in columns) {
        markerStr[columns[m] - 1 + margin] = positionMarker;
      }
      lines.push(markerStr.join(''));
    }
